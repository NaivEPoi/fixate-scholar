...
try {
  ...
  const out = await ev(`(() => {
    ...
    if (layer) for (const a of layer.querySelectorAll("a")) {
      ...
      let txt = "";
      for (const s of div.querySelectorAll("span")) { if (s.querySelector("span:not(.fx-cite-c):not(.fx-ref-c):not(.fx-sp)")) continue; const sr = s.getBoundingClientRect(); if (ov(sr, r) > 0.3*sr.width*sr.height) txt += s.textContent; }
