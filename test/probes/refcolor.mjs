...
export const create = () => ({ T: 0, C: 0, pages: 0, unmeasured: [] });
...
export function add(state, p, r) {
  ...
  state.pages++;
  state.T += r.total; state.C += r.colored;
...
export function summarize(state) {
  return { ok: !(state.T > state.C), out: [`\nTOTAL refs=${state.T} colored=${state.C}`], err: [] };
}
