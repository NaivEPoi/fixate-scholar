...
| `node test/comments.mjs [url] [page]` | comments on highlights: the feature is ON (PDF.js hides it behind a false-by-default preference — vendored patch 7), a note goes onto a highlight through the real UI (editor comment button → dialog → save), the save writes what OTHER readers need (`/Highlight` with `/Contents` and the author's `/T`, a child `/Popup` parented to it, `/F 4`, an `/AP` stream), the text round-trips, and reopening the SAVED bytes in reading mode still shows the note's button, un-covered by the citation hit-target layer, opening the note on click | `COMMENTS: PASS` (exit 0) |
